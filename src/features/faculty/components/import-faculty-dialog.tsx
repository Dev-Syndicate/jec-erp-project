...
function RowErrorTable({ errors }: { errors: ImportRowError[] }) {
  return (
    ...
    >
      ...
      <TableBody>
        {errors.map((e) => (
          <TableRow key={`${e.rowNumber}-${e.staffId}`}>
            <TableCell className="tabular-nums text-muted-foreground">{e.rowNumber}</TableCell>
            <TableCell className="font-mono text-xs">{e.staffId || "—"}</TableCell>
            {/* whitespace-normal: a parse error is a sentence and must wrap. */}
...
function OutcomeTable({ outcomes }: { outcomes: ImportOutcome[] }) {
  // Token-backed, matching the student importer: `created` is a success and
  // --success is what that means everywhere else in the app.
  ...
  return (
    ...
    >
      ...
      <TableBody>
        {outcomes.map((o) => (
          <TableRow key={`${o.rowNumber}-${o.staffId}`}>
            <TableCell className="font-mono text-xs">{o.staffId}</TableCell>
