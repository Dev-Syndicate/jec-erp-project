// /api/branches/[id] — update + delete a single Branch. Super-Admin only (Structure
...
// allowed only when the Branch has no Programs — otherwise the FK would restrict
...
type BranchPatch = { name?: string; code?: string; isActive?: boolean };

function parsePatchBody(body: unknown): { data: BranchPatch } | { error: string } {
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      ...
      if (isNotFound(e)) return Response.json({ error: "Branch not found." }, { status: 404 });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // a Branch with Programs must be deactivated, not deleted.
    const branch = await db.branch.findUnique({
      where: { id },
      include: { _count: { select: { programs: true } } },
    });
    if (!branch) return Response.json({ error: "Branch not found." }, { status: 404 });
    if (branch._count.programs > 0) {
      return Response.json(
        {
          error:
            "This branch has programs built on it. Deactivate it instead of deleting to keep history.",
        },
        { status: 409 },
      );
    }
